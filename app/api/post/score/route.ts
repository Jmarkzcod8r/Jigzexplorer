...
export async function GET() {
  try {
    await dbConnect();
    const scores = await Score.find().sort({ updatedAt: -1 }); // newest first
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ Merge/Update each country instead of overwriting entire object
    Object.entries(countries as Record<string, any>).forEach(([countryName, details]) => {
      updateField[`countries.${countryName}`] = {
        ...
        datePlayed: details.datePlayed ? new Date(details.datePlayed) : new Date(),
      };
    });

    // ✅ Upsert with timestamps automatically handled
